function results = qda(X, y, use_normalization, normalization_method, class_names, feature_names, best_pair, save_prefix)
    ...
    for i = 1:n
        ...
        model = qda_train(Xtrn, ytr);
        y_pred(i) = qda_predict(model, Xten);
    end
    ...
    model_all = qda_train(Xall, y);
    plot_qda_model_2d(model_all, Xall, y, class_names, feature_names, best_pair, save_prefix);
...
function model = qda_train(X, y)
    ...
    model.means = means;
    model.covs = covs;
    model.invCovs = invCovs;
    model.logDets = logDets;
end


function yhat = qda_predict(model, Xtest)
    if size(Xtest,1) == 1
    ...
    for i = 1:C
        mu = model.means(i,:);
        Xm = Xtest - mu;
        quad = sum((Xm * model.invCovs{i}) .* Xm, 2);
        scores(:,i) = -0.5 * model.logDets(i) ...
                    - 0.5 * quad ...
                    + log(model.priors(i));
    end
...
function plot_qda_model_2d(model, X, y, class_names, feature_names, best_pair, save_prefix)
    f1 = best_pair(1);
    f2 = best_pair(2);
    ...
    title('QDA - fronteira de decisao 2D');
